...
`@script`
In Python, we use numpy's pinv function to solve for the least squares approximation.

Let's use numpy to verify the properties of the pseudoinverse

We create a 3-by-2 matrix A and find its inverse, A_plus. 

A_plus times A is very close to the identity matrix. However A times A_plus is completely different.
